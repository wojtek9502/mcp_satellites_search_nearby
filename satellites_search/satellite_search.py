...
class SatelliteSearch:
    ...
    async def calculate_satellites_nearby(self) -> str:
        ...
        if len(single_passes_texts):
            satellites_passes_text = satellites_passes_text + '\n' + '\n'.join(single_passes_texts)
        else:
            satellites_passes_text = f"There are no satellites passes for {self.satellite_name} for your location in the next {self.range_days} days."
