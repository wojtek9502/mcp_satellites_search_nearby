...
# tip: do not try to pass pydantic class with parameters inside, it will cause errors
...
async def get_satellite_passes(
    ...
    min_above_horizon_deg: Annotated[int, Field(description="Minimum satellite altitude above horizon in degrees (default: 20)")] = 20,
) -> str:
    """
        Get satellite passes. Use this tool to calculate satellites passes.
    """
    ...
    satellite_search = SatelliteSearch(
        ...
        elev=elevation_m,
        min_above_horizon_deg=min_above_horizon_deg
    )
